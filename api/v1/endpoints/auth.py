...
class LoginRequest(BaseModel):
    ...
    Legacy single-admin mode: ``password`` only (kept for backward compat).
    """
...
def _get_auth_status_dict(request: Request | None = None) -> dict:
    ...
    if auth_enabled and request:
        ...
        if cookie_val:
            current_user = get_session_user(cookie_val)
...
@router.post(
    ...
    description=(
        ...
        "Multi-user mode uses the SQLite-backed user store; the legacy "
        "single-admin path (password only) is retained for backward compat."
    ),
)
async def auth_login(request: Request, body: LoginRequest):
    """Verify credentials, set cookie on success. Returns 401 or 429 on failure."""
    ...
    # Make sure the SQLite user store is initialised (default admin/123456)
    # before attempting multi-user login.
    try:
        init_user_db()
    except Exception as exc:  # noqa: BLE001
        logger.error("Failed to initialise user store during login: %s", exc, exc_info=True)
    ...
    # Multi-user login path (preferred).
    if username:
        ...
        clear_rate_limit(ip)
        session_val = create_user_session(user["username"], user["role"])
        if not session_val:
            return JSONResponse(
                status_code=500,
                content={"error": "internal_error", "message": "Failed to create session"},
            )
        resp = JSONResponse(content={"ok": True, "user": {"username": user["username"], "role": user["role"]}})
        _set_session_cookie(resp, session_val, request)
        return resp

    # Legacy single-admin path (no username field sent by older clients).
    ...
    resp = JSONResponse(content={"ok": True, "user": {"username": "admin", "role": "admin"}})
    _set_session_cookie(resp, session_val, request)
    return resp
...
@router.post(
    ...
    description="Change password for the current user (multi-user) or admin (legacy). Requires valid session.",
)
async def auth_change_password(request: Request, body: ChangePasswordRequest):
    """Change password. Requires login.

    When the request is in multi-user mode (``request.state.user`` is set by
    the auth middleware), change the current user's password in the SQLite
    user store. Otherwise fall back to the legacy single-admin change flow.
    """
    ...
    if isinstance(current_user, dict) and current_user.get("username"):
        # Multi-user mode: look up by username and change in SQLite.
        from src.user_store import get_user_by_username
        target = get_user_by_username(current_user["username"])
        if target is None:
            return JSONResponse(
                status_code=400,
                content={"error": "user_not_found", "message": "当前用户不存在"},
            )
        err = user_change_password(target["id"], current, new_pwd)
        if err:
            return JSONResponse(
                status_code=400,
                content={"error": "invalid_password", "message": err},
