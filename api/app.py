...
async def app_lifespan(app: FastAPI):
    ...
    # Initialize the multi-user SQLite store (creates ./data/users.db on demand
    # and seeds the default admin/123456 account if there are no users).
    try:
        from src.user_store import init_db as init_user_db
        init_user_db()
    except Exception as exc:  # noqa: BLE001
        logger.error("Failed to initialise user store: %s", exc, exc_info=True)
