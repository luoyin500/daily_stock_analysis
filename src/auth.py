...
def create_session() -> str:
    ...
    Note: This is the legacy single-admin session format retained for
    backward compatibility. New code should prefer :func:`create_user_session`
    which carries the authenticated user's identity in the signed payload.
    """
...
def verify_session(value: str) -> bool:
    ...
    Accepts both the legacy ``nonce.ts.signature`` format and the new
    user-session format produced by :func:`create_user_session`.
    """
    secret = _get_session_secret()
    if not secret or not value:
        return False
    # New format: base64(json({nonce, ts, username, role})).signature
    if value.count(".") == 1:
        return _verify_user_session_value(secret, value)
    # Legacy format: nonce.ts.signature
    parts = value.split(".")
    if len(parts) != 3:
        return False
...
def _b64url_encode(data: bytes) -> str:
    """URL-safe base64 without padding (so the cookie value has no '=' chars)."""
...
def create_user_session(username: str, role: str) -> str:
    """Create a signed session that embeds the authenticated user.

    Format: ``base64url(json({nonce, ts, username, role})).signature``.

    The signature is HMAC-SHA256 over the base64url-encoded payload using the
    shared session secret (same one used by the legacy admin session).
    """
    secret = _get_session_secret()
    if not secret:
        return ""
    ...
    payload_json = _json.dumps(payload_obj, separators=(",", ":"))
    payload_b64 = _b64url_encode(payload_json.encode("utf-8"))
    sig = hmac.new(secret, payload_b64.encode("ascii"), hashlib.sha256).hexdigest()
    return f"{payload_b64}.{sig}"


def _verify_user_session_value(secret: bytes, value: str) -> bool:
    """Verify a user-session cookie value (signature + expiry)."""
    parts = value.split(".")
    if len(parts) != 2:
        return False
...
def get_session_user(value: str) -> Optional[dict]:
    """Return ``{username, role}`` from a user-session cookie, or None.

    Falls back to returning ``{"username": "admin", "role": "admin"}`` for
    legacy admin sessions, so callers can uniformly read the current user.
    """
    if not value:
        return None
    secret = _get_session_secret()
    ...
    if value.count(".") == 1:
        ...
        try:
            ...
            max_age_hours = SESSION_MAX_AGE_HOURS_DEFAULT
        ts = payload.get("ts")
        if not isinstance(ts, (int, float)) or time.time() - float(ts) > max_age_hours * 3600:
            return None
    ...
    # Legacy admin session: still valid => treat as admin.
    if verify_session(value):
