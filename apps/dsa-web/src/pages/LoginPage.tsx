...
import { Lock, Loader2, Cpu, TrendingUp, Network, ShieldCheck, User } from "lucide-react";
        ...
        >
          ...
          <div className="mt-8 flex flex-col items-center">
            <h2 className="text-4xl font-extrabold tracking-tighter text-[var(--login-text-primary)] sm:text-6xl">
              <span className="bg-gradient-to-r from-[var(--login-text-primary)] via-[var(--login-text-primary)] to-[var(--login-text-secondary)] bg-clip-text text-transparent">御西NX</span>
              <span className="bg-gradient-to-r from-[var(--login-brand-start)] to-[var(--login-brand-end)] bg-clip-text text-transparent drop-shadow-[0_0_20px_var(--login-accent-glow)]">量化平台</span>
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                <div className="relative">
                  <User className="pointer-events-none absolute left-3 top-1/2 z-10 h-4 w-4 -translate-y-1/2 text-[var(--login-input-icon)]" />
                  <input
                    id="username"
                    name="username"
                    type="text"
                    autoComplete="username"
                    className="peer h-12 w-full rounded-xl border border-[var(--login-input-border)] bg-[var(--login-input-bg)] pl-10 pr-3 pt-3 text-sm text-[var(--login-input-text)] outline-none transition focus:border-[var(--login-input-border-focus)] focus:ring-2 focus:ring-[var(--login-input-ring)]"
                    placeholder=" "
                    value={username}
                  ...
                  <label
                    htmlFor="username"
                    className="pointer-events-none absolute left-10 top-1/2 z-10 -translate-y-1/2 text-sm text-[var(--login-input-icon)] transition-all peer-focus:top-2.5 peer-focus:left-3 peer-focus:text-[10px] peer-focus:font-medium peer-[:not(:placeholder-shown)]:top-2.5 peer-[:not(:placeholder-shown)]:left-3 peer-[:not(:placeholder-shown)]:text-[10px] peer-[:not(:placeholder-shown)]:font-medium"
                  >
                    {t('login.username')}
                  </label>
                </div>
